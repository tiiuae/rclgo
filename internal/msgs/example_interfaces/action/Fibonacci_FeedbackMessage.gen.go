...
func Fibonacci_FeedbackMessage__Sequence_to_Go(goSlice *[]Fibonacci_FeedbackMessage, cSlice CFibonacci_FeedbackMessage__Sequence) {
	...
	*goSlice = make([]Fibonacci_FeedbackMessage, int64(cSlice.size))
	for i := 0; i < int(cSlice.size); i++ {
		cIdx := (*C.example_interfaces__action__Fibonacci_FeedbackMessage__Sequence)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_example_interfaces__action__Fibonacci_FeedbackMessage * uintptr(i)),
		))
...
func Fibonacci_FeedbackMessage__Sequence_to_C(cSlice *CFibonacci_FeedbackMessage__Sequence, goSlice []Fibonacci_FeedbackMessage) {
	...
	cSlice.data = (*C.example_interfaces__action__Fibonacci_FeedbackMessage)(C.malloc((C.size_t)(C.sizeof_struct_example_interfaces__action__Fibonacci_FeedbackMessage * uintptr(len(goSlice)))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity

	for i, v := range goSlice {
		cIdx := (*C.example_interfaces__action__Fibonacci_FeedbackMessage)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_example_interfaces__action__Fibonacci_FeedbackMessage * uintptr(i)),
		))

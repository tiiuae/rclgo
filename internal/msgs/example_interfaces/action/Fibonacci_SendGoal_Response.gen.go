...
func Fibonacci_SendGoal_Response__Sequence_to_Go(goSlice *[]Fibonacci_SendGoal_Response, cSlice CFibonacci_SendGoal_Response__Sequence) {
	...
	*goSlice = make([]Fibonacci_SendGoal_Response, int64(cSlice.size))
	for i := 0; i < int(cSlice.size); i++ {
		cIdx := (*C.example_interfaces__action__Fibonacci_SendGoal_Response__Sequence)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_example_interfaces__action__Fibonacci_SendGoal_Response * uintptr(i)),
		))
		Fibonacci_SendGoal_ResponseTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(cIdx))
	}
}
func Fibonacci_SendGoal_Response__Sequence_to_C(cSlice *CFibonacci_SendGoal_Response__Sequence, goSlice []Fibonacci_SendGoal_Response) {
	...
	cSlice.data = (*C.example_interfaces__action__Fibonacci_SendGoal_Response)(C.malloc((C.size_t)(C.sizeof_struct_example_interfaces__action__Fibonacci_SendGoal_Response * uintptr(len(goSlice)))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity

	for i, v := range goSlice {
		cIdx := (*C.example_interfaces__action__Fibonacci_SendGoal_Response)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_example_interfaces__action__Fibonacci_SendGoal_Response * uintptr(i)),
		))
		Fibonacci_SendGoal_ResponseTypeSupport.AsCStruct(unsafe.Pointer(cIdx), &v)
	}
}

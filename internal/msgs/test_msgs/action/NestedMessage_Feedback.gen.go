...
func NestedMessage_Feedback__Sequence_to_Go(goSlice *[]NestedMessage_Feedback, cSlice CNestedMessage_Feedback__Sequence) {
	...
	*goSlice = make([]NestedMessage_Feedback, int64(cSlice.size))
	for i := 0; i < int(cSlice.size); i++ {
		cIdx := (*C.test_msgs__action__NestedMessage_Feedback__Sequence)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_test_msgs__action__NestedMessage_Feedback * uintptr(i)),
		))
		NestedMessage_FeedbackTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(cIdx))
	}
}
func NestedMessage_Feedback__Sequence_to_C(cSlice *CNestedMessage_Feedback__Sequence, goSlice []NestedMessage_Feedback) {
	...
	cSlice.data = (*C.test_msgs__action__NestedMessage_Feedback)(C.malloc((C.size_t)(C.sizeof_struct_test_msgs__action__NestedMessage_Feedback * uintptr(len(goSlice)))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity

	for i, v := range goSlice {
		cIdx := (*C.test_msgs__action__NestedMessage_Feedback)(unsafe.Pointer(
			uintptr(unsafe.Pointer(cSlice.data)) + (C.sizeof_struct_test_msgs__action__NestedMessage_Feedback * uintptr(i)),
		))
		NestedMessage_FeedbackTypeSupport.AsCStruct(unsafe.Pointer(cIdx), &v)
	}
}

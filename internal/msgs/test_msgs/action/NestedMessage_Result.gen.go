...
type NestedMessage_Result struct {
	NestedFieldNoPkg test_msgs_msg.Builtins `yaml:"nested_field_no_pkg"`// goal definitionresult definition
	NestedField test_msgs_msg.BasicTypes `yaml:"nested_field"`// goal definitionresult definition
	NestedDifferentPkg builtin_interfaces_msg.Time `yaml:"nested_different_pkg"`// goal definitionresult definition
}
...
type NestedMessage_ResultSubscriptionCallback func(msg *NestedMessage_Result, info *rclgo.RmwMessageInfo, err error)

// NewNestedMessage_ResultSubscription creates and returns a new subscription for the
// NestedMessage_Result
func NewNestedMessage_ResultSubscription(node *rclgo.Node, topic_name string, subscriptionCallback NestedMessage_ResultSubscriptionCallback) (*NestedMessage_ResultSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg NestedMessage_Result
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, NestedMessage_ResultTypeSupport, callback)
	if err != nil {
		return nil, err
	}
...
func (s *NestedMessage_ResultSubscription) TakeMessage(out *NestedMessage_Result) (*rclgo.RmwMessageInfo, error) {
